...
Feature: Shipment Management - Search Shipment 2
  ...
  Scenario: Search Shipment by Filter - Last Inbound Hub (uid:7a2bf3c3-622d-4f31-9851-02ef7797ef1b)
    ...
    When API Operator creates 1 new Hub using data below:
      | name         | GENERATED |
    ...
    When API MM - Operator creates ad hoc trip
      | requestBody | {"origin_hub_id":"{hub-id}","origin_hub_system_id":"sg","destination_hub_id":"{KEY_LIST_OF_CREATED_HUBS[1].id}","destination_hub_system_id":"sg","departure_datetime":"{date: 0 seconds next, yyyy-MM-dd}T23:00:00Z","duration_time":"00:00:30","movement_type":"LAND_HAUL"} |
    ...
    When API MM - Operator creates multiple 1 new shipments with type "LAND_HAUL" from hub id "{hub-id}" to "{KEY_LIST_OF_CREATED_HUBS[1].id}"
