...
Feature: E2E With Trip
  ...
  Scenario: End Inbound Parcel  with Trip in Shipment Destination Hub - Add to New Route - Success Delivery -  e2e
    ...
    Scenario: Hub Inbound Parcel with Trip in Shipment Destination Hub - Transfer Parcel - e2e
      ...
      And API Operator create new route using data below:
        | createRouteRequest | { "zoneId":{zone-id}, "hubId":{hub-id}, "vehicleId":{vehicle-id}, "driverId":{ninja-driver-id} } |
      Then API Driver Ext transfer parcel to another driver
        | to_driver_id     | {ninja-driver-2-id}    |
        | to_driver_hub_id | {hub-id}               |
      And API Driver set credentials "{ninja-driver-2-username}" and "{ninja-driver-2-password}"
      And API Driver collect all his routes
      And API Driver get pickup/delivery waypoint of the created order
      And API Operator Van Inbound parcel
      And API Operator start the route
