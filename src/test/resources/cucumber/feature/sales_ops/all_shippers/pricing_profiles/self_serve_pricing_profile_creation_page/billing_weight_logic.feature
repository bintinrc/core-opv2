...
Feature: Billing Weight Logic
  ...
  Scenario: Upload Bulk Pricing Profiles CSV - Failed Create Pricing Profiles with Billing Weight Logic (uid:eb9203cd-6ab1-496f-961f-73652ccfc1e2)
    ...
    And Operator uploads csv file with below data:
      ...
      | {KEY_LEGACY_SHIPPER_ID} | {KEY_SHIPPER_ID} | {pricing-script-id-all} | {date: 1 days next, d/M/yyyy} | 10.00                | FLAT          | 10  | surcharge | 3              | 4           | 1.5                  | 20                | 10                  | EMPTY                |
      | {KEY_LEGACY_SHIPPER_ID} | {KEY_SHIPPER_ID} | {pricing-script-id-all} | {date: 1 days next, d/M/yyyy} | 10.00                | FLAT          | 10  | surcharge | 3              | 4           | 1.5                  | 20                | 1.5                 | INVALID              |
  ...
  Scenario: Upload Bulk Pricing Profiles CSV With Billing Weight Logic (uid:80e22696-fa30-449c-bcf4-c14bd56e9f0b)
    ...
    And Operator successfully uploads csv file with below data:
      | shipper_id                                 | global_id                            | pricing_script_id       | effective_date                | salesperson_discount | discount_type | rts | rts_type  | cod_percentage | cod_min_fee | insurance_percentage | insurance_min_fee | insurance_threshold | billing_weight_logic |
      | {KEY_LIST_OF_CREATED_SHIPPERS[1].legacyId} | {KEY_LIST_OF_CREATED_SHIPPERS[1].id} | {pricing-script-id-all} | {date: 1 days next, d/M/yyyy} | 10.00                | FLAT          | 20  | surcharge | 3              | 4           | 20                   | 20                | 10                  | LEGACY               |
      | {KEY_LIST_OF_CREATED_SHIPPERS[2].legacyId} | {KEY_LIST_OF_CREATED_SHIPPERS[2].id} | {pricing-script-id-all} | {date: 1 days next, d/M/yyyy} | 10.00                | FLAT          | 20  | discount  | 3              | 4           | 1.5                  | 20                | 1.5                 | GROSS_WEIGHT         |
      | {KEY_LIST_OF_CREATED_SHIPPERS[3].legacyId} | {KEY_LIST_OF_CREATED_SHIPPERS[3].id} | {pricing-script-id-all} | {date: 1 days next, d/M/yyyy} | 10.00                | FLAT          | 20  | surcharge | 3              | 4           | 20                   | 20                | 10                  | SHIPPER_GROSS_WEIGHT |
      | {KEY_LIST_OF_CREATED_SHIPPERS[4].legacyId} | {KEY_LIST_OF_CREATED_SHIPPERS[4].id} | {pricing-script-id-all} | {date: 1 days next, d/M/yyyy} | 10.00                | FLAT          | 20  | discount  | 3              | 4           | 1.5                  | 20                | 1.5                 | STANDARD             |
