...
Feature: Upload CSV Payment From Ninja Van To Shipper (Credit) - Nett COD Remittance
  ...
  Scenario Outline: 1 Account ID linked 1 Shipper - Payment via CSV Upload for Nett COD Remittance with Transaction Amount =  Nett of COD Amount - CSV Has Shipper Legacy ID And Payer Info - ID
    ...
    Examples:
      | source   | account_id                                           | amount  | type | event                | payment_method | transaction_no                                             | payee_name       | payee_account_number                                       | payee_bank |
      | Netsuite | QA-SO-AUTO-TC1-{gradle-current-date-yyyyMMddHHmmsss} | 98.989  | Out  | Nett COD Remittance  | Banking        | QA-SO-AUTO-{KEY_SHIPPER_ID}-{gradle-current-date-yyyyMMdd} | QA-SO-AUTO-Payee | QA-SO-AUTO-{KEY_SHIPPER_ID}-{gradle-current-date-yyyyMMdd} | QA-SO-Bank |