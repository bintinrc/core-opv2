...
Feature: Upload CSV Payment From Shipper To Ninja Van (Debit)
  ...
  Scenario Outline: 1 Account ID linked to 1 Shipper - Payment via CSV Upload from Shipper with exact amount of "Ready" ledger balance - CSV Has Shipper Legacy ID And Payer Info (uid:29cebde6-2502-459b-8988-f472637d949d)
    ...
    Examples:
      ...
      | Netsuite | QA-SO-AUTO-{gradle-current-date-yyyyMMddHHmmsss} | 5.62   | DEBIT | Banking        | QA-SO-AUTO-Payee | QA-SO-AUTO-{KEY_SHIPPER_ID}-{gradle-current-date-yyyyMMdd} | QA-SO-Bank | QA-SO-AUTO-{KEY_SHIPPER_ID}-{gradle-current-date-yyyyMMdd}
