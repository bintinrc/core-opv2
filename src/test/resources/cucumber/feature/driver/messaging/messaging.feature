...
Feature: Messaging
  ...
  Scenario: Search Driver by First Name (uid:f87966e7-8c79-4770-81a0-53157b0a15dc)
    And DB Operator find drivers by "Auto Opv2" first name
    And API Operator find drivers by "Auto Opv2" first name
    Then Drivers found by first name using API and DB are the same
    When Operator opens Messaging panel
    And Operator enters "Auto Opv2" into Add Driver field on Messaging panel
  ...
  Scenario: Search Driver by Driver Type Given Less Than 100 Drivers Listed Use Same Type (uid:f31378ea-f465-4de6-96bf-f4f0cb795e80)
    ...
    And DB Operator find drivers by "OPV2-CORE-OPS" driver type name
    When Operator opens Messaging panel
    And Operator selects "OPV2-CORE-OPS" drivers group on Messaging panel
