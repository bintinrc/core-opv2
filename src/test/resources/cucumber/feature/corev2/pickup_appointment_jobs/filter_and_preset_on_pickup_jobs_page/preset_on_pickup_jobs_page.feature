...
Feature: Preset on pickup jobs page
  ...
  Scenario: Update preset filters on Pickup Jobs page
    Given API Lighthouse - Operator delete Pickup job preset with name = "automationCreateNew"
    Given API Operator create new Preset filters using data below:
      | createPresetFiltersRequest | {"name":"automationCreateNew","value":{"priority":{"value":"All"},"shippers":[],"masterShippers":[],"zones":[],"jobStatus":[{"value":"ready-for-routing","label":"Ready for Routing"},{"value":"routed","label":"Routed"}],"serviceLevel":[],"serviceType":[{"value":"Scheduled"}]}} |
    Given Operator goes to Pickup Jobs Page
    When Operator select Preset with name = "automationCreateNew" in pickup appointment
    Then QA verify filters on Pickup Jobs page are shown
      | dateStart      | {gradle-current-date-dd/MM/yyyy} |
      | priority       | All                              |
      | jobServiceType | Scheduled                        |
