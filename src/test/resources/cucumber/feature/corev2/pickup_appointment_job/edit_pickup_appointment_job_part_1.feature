@PickupAppointment @JobPage @Edit @Part1
Feature: Edit Pickup Appointment Job Part 1
  ...
  Scenario: Edit pickup appointment job - add jobs tag
    ...
    When Operator select job tag = "PRIOR" in Job Tags Field
    ...
    Then QA verify successful message is displayed with the job's date and time
      ...
      | timeRange | 09:00 - 12:00                  |
    And QA verify the job's tag in control_qa_gl.pickup_appointment_jobs_pickup_tags is updated
      | tag | 287 |
    And Operator verify the dialog shows the job tag
      | date | {gradle-next-1-day-yyyy-MM-dd} |
      | tag  | PRIOR                          |
  ...
  Scenario: Edit pickup appointment job - remove jobs tag
    ...
    And QA verify the job's tag in control_qa_gl.pickup_appointment_jobs_pickup_tags is removed

    And Operator verify the dialog not shows the job tag
      | date | {gradle-next-1-day-yyyy-MM-dd} |
      | tag  | PRIOR                          |
