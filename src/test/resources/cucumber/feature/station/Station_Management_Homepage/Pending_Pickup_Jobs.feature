...
Feature: Pending Pickup Jobs
  ...
  @ForceSuccessOrder @ForceSuccessReservationByApi @DeleteOrArchiveRoute @TimeBased @SystemIdNotSg @default-vn @Checkname
  Scenario Outline: Success Pickup All parcel in Reservation
    Given Operator loads Operator portal home page
    Given DB Operator delete Station Pending Pickup records for the hub "<HubId>"
    And API Operator calls pending pickup job trigger for station
  ...
  Scenario Outline: Success Pickup Address with No jobs with New Reservation Created by OC
    ...
    And API Driver - Driver submit POD:
      ...
      | jobType    | RESERVATION                                                                                             |
      | jobMode    | PICK_UP                                                                                                 |
      | parcels    | [{ "tracking_id": "{KEY_LIST_OF_CREATED_ORDERS[1].trackingId}", "action":"FAIL","failure_reason_id":1}] |
      | jobAction  | FAIL                                                                                                    |
      | routes     | KEY_DRIVER_ROUTES                                                                                       |
    And API Operator calls pending pickup job trigger for station
    Given Operator loads Operator portal home page
  ...
  @ForceSuccessOrder @ForceSuccessReservationByApi @DeleteOrArchiveRoute @TimeBased @SystemIdNotSg @default-vn @Ddebug
