...
public class EditOrderSteps extends AbstractSteps {
    ...
    public void operatorVerifyOrderEventsWithDescriptionOnEditOrderPage(List<Map<String, String>> data) {
        data.forEach(eventData -> {
            OrderEvent expectedEvent = new OrderEvent(resolveKeyValues(eventData));
            OrderEvent actualEvent = editOrderPage.eventsTable().readAllEntities().stream()
                    .filter(event -> (equalsIgnoreCase(event.getName(), expectedEvent.getName()) && equalsIgnoreCase(event.getDescription(), expectedEvent.getDescription())))
                    .findAny()
                    .orElse(null);
            if (actualEvent == null) {
                pause5s();
