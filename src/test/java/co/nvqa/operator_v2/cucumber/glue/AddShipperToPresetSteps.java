...
public class AddShipperToPresetSteps extends AbstractSteps
{
    ...
    public void operatorValidatesFilterValues(Map<String, String> data)
    {
        ...
        {
            assertEquals("Shipper Creation Date from", value, addShipperToPresetPage.shipperCreationDateFilter.getValueFrom());
        }
        value = data.get("shipperCreationDateTo");
        if (StringUtils.isNotBlank(value))
        {
            assertEquals("Shipper Creation Date to", value, addShipperToPresetPage.shipperCreationDateFilter.getValueTo());
        }
    }
    ...
    public void operatorAppliesFiltersValues(Map<String, String> data)
    {
        ...
        if (StringUtils.isNotBlank(shipperCreationDateFrom) && StringUtils.isNotBlank(shipperCreationDateTo))
        {
            addShipperToPresetPage.shipperCreationDateFilter.setInterval(shipperCreationDateFrom, shipperCreationDateTo);
    ...
    public void operatorAppliesFilter(String filter, String columnName)
    {
        ...
        String columnId = AddShipperToPresetPage.ShippersTable.COLUMN_IDS_BY_NAME.get(columnName.trim().toLowerCase());
    ...
    public void operatorVerifyRecords(List<Map<String, String>> listOfData)
    {
        ...
        for (Map<String, String> listOfDatum : listOfData)
        {
            ...
            }).findFirst().orElseThrow(() -> new AssertionError("Could not find a row with data " + expected.toMap()));
        }
    }
    ...
    public void operatorAddsShipperToPreset(Map<String, String> data)
    {
        ...
        {
            addShipperToPresetPage.shippersTable.filterByColumn(AddShipperToPresetPage.ShippersTable.COLUMN_SHIPPER_NAME, shipperName);
        }
    ...
    {
        addShipperToPresetPage.verifyNotificationWithMessage("Date range cannot be greater than 7 days");
    }
    ...
    public void operatorVerifyAllRowsAreUnselected()
    {
        ...
        for (int i = 1; i <= rowsCount; i++)
        {
            assertFalse("Is row" + i + " selected", addShipperToPresetPage.shippersTable.isRowSelected(i));
        }
    }
