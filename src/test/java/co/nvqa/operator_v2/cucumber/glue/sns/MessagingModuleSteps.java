...
public class MessagingModuleSteps extends AbstractSteps {
    ...
    @And("Search for created order using {string}")
    public void searchForCreatedOrderUsingTrackingId(String trackingId) {
        trackingId = resolveValue(trackingId);
        messagingModulePage.searchSmsSentHistory(trackingId);
    }

    @Then("Operator Verify data is shown in history table for {string}")
    public void verifyDataIsShownInHistoryTable(String trackingId) {
        trackingId = resolveValue(trackingId);
        messagingModulePage.verifyDataInHistoryTable(trackingId);
    }
}