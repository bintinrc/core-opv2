...
public class BulkOrdersSteps extends AbstractSteps
{
    ...
    public void operatorVerifyCreatedBulkOrder() throws InstantiationException, IllegalAccessException
    {
        ...
        String createdAt = DateUtil.DATE_FORMATTER.format(DateUtil.DATE_TIME_FORMATTER.parse(actualBulkOrderInfo.getCreatedAt()));
    ...
    public void operatorVerifyDetailsOfCreatedBulkOrder()
    {
        ...
        Assert.assertEquals("Number of orders in Bulk", expectedBulkOrderInfo.getOrders().size(), actualOrders.size());
    ...
    public void operatorPrintCreatedBulkOrder()
    {
        ...
        Assert.assertEquals("Confirmation text", "Are you sure to print all label for orders?", bulkOrdersPage.printConfirmationDialog.message.getText());
        Assert.assertTrue("Print button enable state", bulkOrdersPage.printConfirmationDialog.printButton.isEnabled());
    }
}
