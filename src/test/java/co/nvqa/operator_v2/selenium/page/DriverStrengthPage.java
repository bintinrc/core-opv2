...
public class DriverStrengthPage extends OperatorV2SimplePage {
  ...
  public void filteredBy(String type) {
    ...
    if ("zone".equals(type)) {
      ...
      sendKeys(
          "//th[contains(@class, 'zone-preferences-zone-id')]/nv-search-input-filter/md-input-container/div/input",
          filterKey);
    } else if ("driver-type".equals(type)) {
      filterKey = driverType != null ? driverType : "Ops";
      sendKeys(
          "//th[contains(@class, 'driver-type')]/nv-search-input-filter/md-input-container/div/input",
          filterKey);
    }
  ...
  public void searchDriver() {
    ...
    sendKeys(
        "//th[contains(@class, 'username')]/nv-search-input-filter/md-input-container/div/input",
        driverUsername);
  }
    ...
    for (WebElement d : listDriver) {
      ...
      for (WebElement e : el) {
        if (e.getText().trim().length() > 0) {
          if (e.getText().trim()
              .equalsIgnoreCase("Driver " + SingletonStorage.getInstance().getTmpId())) {
            isFound = true;
            break;
          }
        }
  ...
  public void changeComingStatus() {
    WebElement firstDriver = findElementsByXpath(
        "//tr[@md-virtual-repeat='driver in getTableData()']").get(0);
  ...
  public void clickViewContactButton() {
    click(
        "//tr[@md-virtual-repeat='driver in getTableData()']/td[contains(@class, 'actions column-locked-right')]/md-menu/button");
    pause1s();
  ...
  public void enterDefaultValue() {
    ...
    // Comments.
    sendKeys("//textarea[@aria-label='Comments']",
        "This driver is created by \"Automation Test\" for testing purpose.");
  ...
  public void editNewDriver() {
    click(
        "//tr[@md-virtual-repeat='driver in getTableData()']/td[contains(@class, 'actions column-locked-right')]/nv-icon-button[1]/button");
    pause1s();
    sendKeys("//textarea[@aria-label='Comments']",
        "This driver is created by \"Automation Test\" for testing purpose. [EDITED]");
  ...
  public void deleteNewDriver() {
    click(
        "//tr[@md-virtual-repeat='driver in getTableData()']/td[contains(@class, 'actions column-locked-right')]/nv-icon-button[2]/button");
    pause1s();
  ...
  private void changeComingStatusState(WebElement el) {
    pause1s();
    el.findElement(By.xpath("//td[contains(@class, 'availability')]/nv-toggle-button/button"))
        .click();
    pause1s();
  }

  private String getComingStatusState(WebElement el) {
    return el.findElement(By.xpath("//td[contains(@class, 'availability')]/nv-toggle-button"))
        .getAttribute("md-theme");
  }
