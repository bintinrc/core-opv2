...
public class DpPage extends OperatorV2SimplePage {
  ...
  public void downloadFile(String type) {
    ...
    click("//div[@filename='" + BTN_NAME_FILENAME.get(type)
        + "']/nv-download-csv-button/div/nv-api-text-button/button");
    pause1s();
  }
    ...
    switch (type) {
      ...
      case "edit dp-partners": {
        ...
        expectedValue = String
            .format("No restrictions enforced. [%s]", SingletonStorage.getInstance().getTmpId());
        break;
      }
      case "add dps": {
        ngRepeat = "dp in $data";
      ...
      case "edit dps": {
        ...
        expectedValue = String
            .format("No directions provided. [%s]", SingletonStorage.getInstance().getTmpId());
        break;
      }
      case "add dp-users": {
        ngRepeat = "dpUser in $data";
  ...
  public void enterDefaultValue(String type) {
    switch (type) {
      case "dp-partners": {
        ...
        sendKeys("//input[@type='text'][@aria-label='Partner Name']",
            String.format("Partner %s", tmpId));
        ...
        sendKeys("//input[@type='email'][@aria-label='POC Email']",
            String.format("%s@poc.co", tmpId));
        sendKeys("//textarea[@name='restrictions'][@aria-label='Restrictions']", "NA");
        break;
      }
      case "dps": {
        sendKeys("//input[@type='text'][@aria-label='Name']",
            String.format("DP %s", SingletonStorage.getInstance().getTmpId()));
        sendKeys("//input[@type='text'][@aria-label='Shortname']",
            String.format("DP%s", SingletonStorage.getInstance().getTmpId()));
        sendKeys("//input[@type='tel'][@aria-label='Contact No.']",
            String.format("+65 %s", SingletonStorage.getInstance().getTmpId()));
        sendKeys("//div[label[text()='Shipper Account']]//input", "QA\n");
        sendKeys("//input[@type='text'][@aria-label='Address Line 1']",
            "Jl. Utan Kayu Raya No. 76");
      ...
      case "dp-users": {
        ...
        sendKeys("//input[@type='text'][@aria-label='Last Name']",
            SingletonStorage.getInstance().getTmpId());
        sendKeys("//input[@type='tel'][@aria-label='Contact No.']",
            String.format("+65 %s", SingletonStorage.getInstance().getTmpId()));
        sendKeys("//input[@type='email'][@aria-label='Email']",
            String.format("%s@poc.co", SingletonStorage.getInstance().getTmpId()));
        sendKeys("//input[@type='text'][@aria-label='Username']",
            String.format("user%s", SingletonStorage.getInstance().getTmpId()));
  ...
  public void clickEditBtn(String type) {
    ...
    String expectedValue = findElementByXpath(
  ...
  /**
   * refer to the data-title-text attribute on the cell. since class are inconsistent and data-title
   * value are too long.
   * <p>
   * e.g.: <tr ng-repeat="dpPartner in $data"> <td data-title="'commons.id' | translate"
   * sortable="'id'" class="id" data-title-text="Id"> 1223 </td> <td data-title="'commons.name' |
   * translate" sortable="'name'" class="name" data-title-text="Name"> Partner 1505289247109 </td>
   * <td data-title="'container.dp-administration.dp-partners.header-poc-name' | translate"
   * sortable="'pocName'" class="poc-name" data-title-text="POC Name"> Poc 1505289247109 </td> <td
   * data-title="'container.dp-administration.dp-partners.header-poc-no' | translate"
   * sortable="'pocTel'" class="poc-tel" data-title-text="POC No."> +65 1505289247109 </td> <td
   * data-title="'container.dp-administration.dp-partners.header-poc-email' | translate"
   * sortable="'pocEmail'" class="poc-email" data-title-text="POC Email"> 1505289247109@poc.co </td>
   * <td data-title="'container.dp-administration.dp-partners.header-restrictions' | translate"
   * data-title-text="Restrictions"> No restrictions enforced. [1505289247109] </td>
   * data-title="'commons.actions' | translate" class="actions" data-title-text="Actions">
  ...
  public String getTextOnTable(String ngRepeat, int rowNumber, String dataTitleText) {
    ...
    WebElement we = findElementByXpath(String
        .format("//tr[@ng-repeat='%s'][%d]/td[@data-title-text='%s']", ngRepeat, rowNumber,
            dataTitleText));
