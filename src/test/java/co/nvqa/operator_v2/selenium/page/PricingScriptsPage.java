...
public class PricingScriptsPage extends OperatorV2SimplePage {
  ...
  public void createScript(String scriptName, String description, String script) {
    ...
    waitUntilVisibilityOfElementLocated(
        "//md-dialog[contains(@class, 'pricing-script-linked-shippers-dialog')]"); // Wait until Link Shippers dialog appear.
    clickNvIconButtonByName("Cancel");
  }

  public String createDefaultScriptIfNotExists(String scriptName, String scriptDescription,
      String script) {
        /*
  ...
  public void updateScript(int rowNumber, String newScriptName, String newDescription,
    ...
    if (newScript != null) {
      ...
      String oldScript = getAceEditorValue()
  ...
  public void searchScript(String scriptName) {
    sendKeys(DEFAULT_MAX_RETRY_FOR_STALE_ELEMENT_REFERENCE, "//th[@column='name']//input",
        scriptName);
    pause1s();
  }
  ...
  public void simulateRunTest(String deliveryType, String orderType, String timeslotType,
      String size, String weight, String insuredValue, String codValue) {
    openEditDialogAndWaitUntilLoadingIsDone();
    selectValueFromMdSelectMenu(
        "//md-input-container[@label='container.pricing-scripts.description-delivery-type']",
        String.format("//md-option[@value='%s']", deliveryType));
    selectValueFromMdSelectMenu(
        "//md-input-container[@label='container.pricing-scripts.description-order-type']",
        String.format("//md-option[@value='%s']", orderType));
    selectValueFromMdSelectMenu(
        "//md-input-container[@label='container.pricing-scripts.description-time-slot-type']",
        String.format("//md-option[@value='%s']", timeslotType));
    selectValueFromMdSelectMenu("//md-input-container[@label='commons.size']",
        String.format("//md-option[@value='%s']", size));
    sendKeysById("commons.weight", weight);
    clickNvApiTextButtonByNameAndWaitUntilDone("container.pricing-scripts.run-check");
  }

  public String linkPricingScriptsToShipper(String defaultScriptName1, String defaultScriptName2,
      String shipperName) {
    ...
    for (String script : scriptToTest) {
      ...
      if (!isPricingScriptsContainShipper(shipperName)) {
        ...
        if (isElementExistFast(
            "//md-dialog[@aria-label='ErrorUnexpected error']/md-dialog-content/h2[text()='Error']")) {
          pause100ms();
          click(
              "//md-dialog[@aria-label='ErrorUnexpected error']/md-dialog-actions/button/span[text()='Close']");
          pause100ms();
  ...
  private String executeJavascript(String script) {
    ...
    if (getWebDriver() instanceof JavascriptExecutor) {
      ...
      throw new NvTestRuntimeException(
          "WebDriver is not instance of JavascriptExecutor. Cannot execute method executeScript.");
    }
