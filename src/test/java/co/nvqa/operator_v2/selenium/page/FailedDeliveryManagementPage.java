...
public class FailedDeliveryManagementPage extends OperatorV2SimplePage
{
    ...
    {
        failedDeliveriesTable.filterByColumn(CommonParcelManagementPage.FailedDeliveriesTable.COLUMN_TRACKING_ID, trackingId);
    ...
    {
        failedDeliveriesTable.filterByColumn(CommonParcelManagementPage.FailedDeliveriesTable.COLUMN_TRACKING_ID, trackingId);
        failedDeliveriesTable.clickActionButton(1, CommonParcelManagementPage.FailedDeliveriesTable.ACTION_RTS);
    ...
    public void verifyFailedDeliveryOrderIsListed(Order order, FailureReason expectedFailureReason)
    {
        ...
        assertEquals("Failure Comments", expectedFailureReason.getDescription(), actual.getFailureReasonComments());
        assertEquals("Failure Reason", expectedFailureReason.getFailureReasonCodeDescription(), actual.getFailureReasonCodeDescription());
    }
    ...
    public void verifyOrderIsRemovedFromTableAfterReschedule(String trackingId)
    {
        ...
        waitWhilePageIsLoading();
        failedDeliveriesTable.filterByColumn(FailedDeliveriesTable.COLUMN_TRACKING_ID, trackingId);
        assertTrue(f("Tracking ID '%s' is still listed on failed order list.", trackingId), failedDeliveriesTable.isEmpty());
    }
}