...
public class CreateOrEditJobPage extends PageElement {
    ...
    public final String NOTIFICATION_MESSAGE_LOCATOR = "div.ant-notification-notice-message";
    ...
    public boolean isDeleteButtonByJobIdDisplayed(String jobId) {
        return webDriver.findElement(By.cssSelector(f(DELETE_BUTTON_IN_CALENDAR_LOCATOR, jobId))).isDisplayed();
    }

    public boolean isEditButtonByJobIdDisplayed(String jobId) {
        return webDriver.findElement(By.cssSelector(f(EDIT_BUTTON_IN_CALENDAR_LOCATOR, jobId))).isDisplayed();
    }
    ...
    public boolean isSelectedTimeRangeWebElementDisplayed(String timeRange) {
        return webDriver.findElement(By.cssSelector(f(SELECTED_TIME_RANGE_LOCATOR, timeRange))).isDisplayed();
    }
    ...
    public String getTextFromNotificationMessage() {
        return getWebDriver().findElement(By.cssSelector(NOTIFICATION_MESSAGE_LOCATOR)).getAttribute(TEXT_CONTENT);
    }

    public String getTextFromNotificationDescription() {
        return getWebDriver().findElement(By.cssSelector(NOTIFICATION_DESCRIPTION_LOCATOR)).getAttribute(TEXT_CONTENT);
    }

    public WebElement getCalendarWebElementByDate(String date) {
        return webElement.findElement(By.xpath(f(CALENDAR_WEB_ELEMENT_LOCATOR, getDayFromFullDate(date))));
    }

    public boolean isTagDisplayedOnPickupJobByDate(String date, String tag) {
        moveToElement(getCalendarWebElementByDate(date));
        return getWebDriver().findElements(By.cssSelector(VISIBLE_SPAN_TAG_LOCATOR)).stream().map(WebElement::getText).collect(Collectors.toList()).contains(tag);
    }
    ...
    public String getColorAttributeInPickupJobFromCalendar(String date) {
        return webElement.findElement(By.xpath(f(STYLE_COLOR_LOCATOR, getDayFromFullDate(date))))
                .getAttribute(STYLE);
    }
