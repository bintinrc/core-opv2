...
public class SamedayRouteEnginePage extends OperatorV2SimplePage {
  ...
  public void selectFleetType1OperatingHoursStart(String operatingHoursStart) {
    ...
    pause100ms();
    clickf(
        "//div[@aria-hidden='false']/md-select-menu/md-content/md-option/div[contains(text(), '%s')]",
        operatingHoursStart);
  }
  ...
  public void verifyWaypointDetailContent(String trackingId, String routeGroupName) {
    ...
    String trackingIdData = getTextOnTableWithMdVirtualRepeat(1, "tracking_id",
        "route in ctrl.routeResponse.solution.routes");
    assertEquals(trackingId, trackingIdData);

    String trackingIdData2 = getTextOnTableWithMdVirtualRepeat(2, "tracking_id",
        "route in ctrl.routeResponse.solution.routes");
    assertEquals(trackingId, trackingIdData2);

    // Check the number of waypoint.
    String waypointTotal = getText(
        "//md-dialog[contains(@class, 'nv-route-detail-dialog')]/md-dialog-content/div[1]/div[2]/p");
  ...
  public void downloadCsvOnWaypointDetails(String trackingId) throws IOException {
    String routeName = "route-detail-" + getText(
        "//md-dialog[contains(@class, 'nv-route-detail-dialog')]/md-dialog-content/div[1]/div[1]/p/b");
    ...
    List<String> lines = Files
        .readAllLines(Paths.get(TestConstants.TEMP_DIR + "/" + routeName + ".csv"),
  ...
  public void openUnroutedDetailDialog() {
    ...
    waitUntilVisibilityOfElementLocated(
        "//md-dialog[contains(@class, 'nv-unrouted-detail-dialog')]");
    pause3s();
  }

  public void verifyUnroutedDetailDialog() {
    String unroutedCount = getText(
        "//md-dialog[contains(@class, 'nv-unrouted-detail-dialog')]/md-dialog-content/div[1]/div[1]/p/b");
  ...
  public void clickUpdateTimeslotBtn() {
    ...
    waitUntilInvisibilityOfElementLocated(
        "//nv-api-text-button[@on-click='ctrl.onSaveSuggestedRoute()']//md-progress-circular");
  }

  public String getWaypointTrackingIds() {
    StringBuilder sb = new StringBuilder();
    int waypointTotal = Integer.valueOf(getText(
        "//md-dialog[contains(@class, 'nv-route-detail-dialog')]/md-dialog-content/div[1]/div[2]/p"));

    for (int i = 0; i < waypointTotal; i++) {
      String trackingId = getTextOnTableWithMdVirtualRepeat(i + 1, "tracking_id",
          "wp in getTableData()");
