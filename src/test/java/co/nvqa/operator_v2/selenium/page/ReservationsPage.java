...
public class ReservationsPage extends OperatorV2SimplePage {
  ...
  private String getNextDateCellXpath() {
    return f("//nv-calendar//div[@ng-repeat='day in week track by $index'][@tabindex='%d']",
        getNextDateNumber());
  }
  ...
  public void createNewReservation(String shipperName, Address address, Reservation reservation,
      String timeslot) {
    String completeAddress = address.getAddress1() + ' ' + address.getAddress2();

    shipper.selectValue(shipperName);
    retryIfRuntimeExceptionOccurred(() -> this.address.selectValue(completeAddress),
        "Select address");
    waitUntilInvisibilityOfElementLocated(
        getNextDateCellXpath() + "//div[contains(@style, 'inherit')]/md-progress-circular");
  ...
  public void verifyReservationIsCreatedSuccessfully(String expectedTimeslotTextOnCalendar) {
    boolean isNextDateCellLabeledAsReserved = isElementExist(
        getNextDateCellXpath() + "//div[text()='Reserved']");
    assertTrue(
        f("Reservation is not created. Label 'Reserved' is not found at the calendar on date %d.",
            getNextDateNumber()), isNextDateCellLabeledAsReserved);
  ...
  public void editReservation(String shipperName, Address address, Reservation reservation,
    ...
    waitUntilInvisibilityOfElementLocated(
        "//div[@id='toast-container']//div[@class='toast-message']/div[@class='toast-right']/div[@class='toast-top']/div[text()='Reservations updated']",
  ...
  public void verifyReservationIsUpdatedSuccessfully(String expectedTimeslotTextOnCalendar) {
    boolean isNextDateCellLabeledAsReserved = isElementExist(
        getNextDateCellXpath() + "//div[text()='Reserved']");
    assertTrue(
        f("Reservation is not updated. Label 'Reserved' is not found at the calendar on date %d.",
            getNextDateNumber()), isNextDateCellLabeledAsReserved);
  ...
  public void deleteReservation(String shipperName, Address address, Reservation reservation) {
    ...
    waitUntilInvisibilityOfElementLocated(
        "//div[@id='toast-container']//div[@class='toast-message']/div[@class='toast-right']/div[@class='toast-top']/div[text()='Reservations deleted']",
        TestConstants.VERY_LONG_WAIT_FOR_TOAST);
  }
  ...
  public void verifyReservationIsDeletedSuccessfully() {
    ...
    assertFalse(
        f("Reservation is not deleted. Label 'Reserved' is still found at the calendar on date %d.",
            getNextDateNumber()), isNextDateCellLabeledAsReserved);
  }
}
