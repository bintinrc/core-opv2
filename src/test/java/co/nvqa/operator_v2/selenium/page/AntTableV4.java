 ...
 */
public class AntTableV4<T extends DataEntity<?>> extends AbstractTable<T> {

  private static final String CELL_LOCATOR_PATTERN = "(//div[contains(@class,'virtual-table')]//div[@data-datakey='%2$s'])[%1$s]";
  ...
  private static final String COLUMN_TEXT_FILTER_LOCATOR_PATTERN = "//input[@data-testid='virtual-table.%s.header.filter']";
  ...
  public AntTableV4(WebDriver webDriver) {
    super(webDriver);
  ...
  protected String getTextOnTable(int rowNumber, String columnDataClass) {
    ...
    if (StringUtils.isNotBlank(tableLocator)) {
      xpath = "(" + tableLocator + xpath.substring(1);
    }
  ...
  public int getRowsCount() {
    String xpath = f(CELL_LOCATOR_PATTERN, "*", columnLocators.values().iterator().next());
    if (StringUtils.isNotBlank(tableLocator)) {
  ...
  public AbstractTable<T> filterByColumn(String columnId, String value) {
    ...
    if (isElementExistWait0Second(xpath)) {
      ...
      if (isElementExistWait0Second(xpath)) {
        ForceClearTextBox input = new ForceClearTextBox(getWebDriver(), xpath);
