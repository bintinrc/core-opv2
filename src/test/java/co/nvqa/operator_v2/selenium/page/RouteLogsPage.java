...
public class RouteLogsPage extends OperatorV2SimplePage {
  ...
  /**
   * This method will create object Route and put it to CreateRouteParams if the route is created
   * successfully.
   *
  ...
  public void createNewRoute(CreateRouteParams createRouteParams) {
    waitUntilPageLoaded();
    setFilterAndLoadSelection(createRouteParams.getRouteDate(), createRouteParams.getRouteDate(),
        createRouteParams.getHubName());
  ...
  public void createMultipleRoute(List<CreateRouteParams> listOfCreateRouteParams) {
    ...
    for (int i = 0; i < listOfCreateRouteParamsSize; i++) {
      sendKeys(String.format("(//textarea[@id='comments'])[%d]", (i + 1)),
          listOfCreateRouteParams.get(i).getComments());
    }
  ...
  public void verifyRouteIsFoundAndInfoIsCorrect(CreateRouteParams createRouteParams) {
    ...
    assertEquals("Route Date", YYYY_MM_DD_SDF.format(createRouteParams.getRouteDate()),
        actualRouteDate);
  ...
  public void bulkEditDetails(List<CreateRouteParams> listOfCreateRouteParams) {
    ...
    selectMultipleValuesFromMdSelectById("commons.model.route-tags",
        createRouteParams.getRouteTags());
    selectValueFromNvAutocompleteByPossibleOptions("hubsSelectionOptions",
        createRouteParams.getHubName());
    selectValueFromNvAutocompleteByPossibleOptions("driversSelectionOptions",
        normalizeNinjaDriverName(createRouteParams.getNinjaDriverName()));
    selectValueFromNvAutocompleteByPossibleOptions("vehiclesSelectionOptions",
        createRouteParams.getVehicleName());
    clickNvButtonSaveByNameAndWaitUntilDone("commons.save-changes");
    waitUntilInvisibilityOfToast("Route(s) Edited");
  }

  public void editDriverTypesOfSelectedRoute(List<CreateRouteParams> listOfCreateRouteParams,
      DriverTypeParams driverTypeParams) {
    checkMultipleRows(listOfCreateRouteParams);
    selectAction(ACTION_EDIT_DRIVER_TYPES_OF_SELECTED);
    selectMultipleValuesFromMdSelectById("container.route-logs.select-driver-types",
        XpathTextMode.EXACT, String.valueOf(driverTypeParams.getDriverTypeId()));
  ...
  public void verifyTransactionsOfMultipleRoutesIsMergedSuccessfully(
      List<CreateRouteParams> listOfCreateRouteParams) {
    ...
    String[] arrayOfRouteIdAsString = listOfCreateRouteParams.stream()
        .map(createRouteParams -> String.valueOf(createRouteParams.getCreatedRoute().getId()))
        .toArray(String[]::new);
    String[] actualMergedRoutes = toastBottomText.replaceFirst("Route ", "").split(", ");
    assertThat("Expected Tracking ID not found.", Arrays.asList(actualMergedRoutes),
        hasItems(arrayOfRouteIdAsString));
  }
  ...
  public void verifyPrintedPasswordsOfSelectedRoutesInfoIsCorrect(
    ...
    for (CreateRouteParams createRouteParams : listOfCreateRouteParams) {
      ...
      assertEquals(String.format("Route Password for Route ID = %d", expectedRouteId),
          route.getRoutePassword(), selectedRoutePassword.getRoutePassword());
    }
  }
  ...
  public void verifyMultipleRoutesIsArchivedSuccessfully(
      List<CreateRouteParams> listOfCreateRouteParams) {
    for (CreateRouteParams createRouteParams : listOfCreateRouteParams) {
  ...
  public void verifyMultipleRoutesIsUnarchivedSuccessfully(
      List<CreateRouteParams> listOfCreateRouteParams) {
    for (CreateRouteParams createRouteParams : listOfCreateRouteParams) {
      retryIfRuntimeExceptionOccurred(() ->
      {
  ...
  public void verifyMultipleRoutesIsDeletedSuccessfully(
      List<CreateRouteParams> listOfCreateRouteParams) {
    for (CreateRouteParams createRouteParams : listOfCreateRouteParams) {
      retryIfRuntimeExceptionOccurred(() ->
      {
        ...
        try {
          assertTrue(String.format("Route with ID = %d is still exists on table.", routeId),
              isTableEmpty);
        } catch (AssertionError ex) {
          editFilterAndLoadSelection();
  ...
  public void editAssignedDriver(String newDriverName) {
    sendKeys("//div/label[text()='Assigned Driver']/following-sibling::nv-autocomplete//input",
        newDriverName);
    pause1s();
    clickf(
  ...
  public void selectTag(long routeId, String newTag) {
    ...
    click(
        "//nv-table-description/div/div/span[text()='Showing']"); //Click on random element to close 'Select Tag' dialog.
    pause200ms();
  ...
  public void loadAndVerifyRoute(Date filterRouteDateFrom, Date filterRouteDateTo,
      String filterHubName, long routeId) {
